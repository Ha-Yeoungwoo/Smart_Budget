...
void renderInputScreen() {
    ...
    for (int i = 0; i < static_cast<int>(categories.size()); ++i) {
        SDL_Rect categoryRect = { 300 + i * 110, 180, 100, 30 };

        if (selectedCategory >= 0 && selectedCategory < static_cast<int>(categories.size())) {
            // selectedCategory가 유효한 범위 내에 있을 때
...
void renderTransactionsScreen() {
    ...
    int endIdx = std::min(static_cast<int>(startIdx) + ITEMS_PER_PAGE, 
    ...
    if (!currentTransactions.empty()) {
        int totalPages = (static_cast<int>(currentTransactions.size()) + ITEMS_PER_PAGE - 1) / ITEMS_PER_PAGE;
